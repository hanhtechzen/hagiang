<?php  
	...
	/* Lấy bài viết tĩnh */
	$static = $d->rawQueryOne("select id, type, ten$lang, noidung$lang, photo,gia,khoihanh,lichtrinh$lang,mota$lang from #_static where type = ? limit 0,1",array($type));
	$hinhanhsp = $d->rawQuery("select photo from #_gallery where hienthi=1 and id_photo = ? and com='static' and type = ? and kind='man' and val = ? order by stt,id desc",array($static['id'],$type,$type));
