<?php  
	...
	{
		/* Lấy cấp 1 detail */
		$pro_list = $d->rawQueryOne("select id, ten$lang, tenkhongdauvi, tenkhongdauen, type, photo, options,mota$lang,tieude$lang,noidung$lang,masp,thoigian,gia from #_product_list where id = ? and type = ? limit 0,1",array($idl,$type));
		...
		if(empty($_SESSION['khachsan'])){
			$_SESSION['khachsan'][] = $pro_list['id'];
		}else{
			if(!in_array($pro_list['id'], $_SESSION['khachsan'])) $_SESSION['khachsan'][] = $pro_list['id'];
		}
	}
	else if($idc!='')
	{
	...
	else
	{
		...
		$startpoint = ($curPage * $per_page) - $per_page;
		$limit = " limit ".$startpoint.",".$per_page;
		$sql = "select photo, ten$lang, tenkhongdauvi, tenkhongdauen, mota$lang,id from #_product_list where $where order by stt,id desc $limit";
		$product = $d->rawQuery($sql,$params);
		...
		if(!empty($_SESSION['khachsan'])){
			$sql = "select photo, ten$lang, tenkhongdauvi, tenkhongdauen, mota$lang,id from #_product_list where hienthi=1 and type = ? and id in(".implode(',',$_SESSION['khachsan']).") order by stt,id desc";
