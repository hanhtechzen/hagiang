<?php
	...
	function save_seopage()
	{
		...
		if(!empty($dataSeo)) { foreach($dataSeo as $column => $value) $dataSeo[$column] = htmlspecialchars($value); }
		$dataSeo['type'] = $type;
		...
		if(isset($_FILES['file'])){
			...
			{
				$dataSeo['photo'] = $photo;
				$row = $d->rawQueryOne("select id, photo from #_seopage where type = ?",array($type));
				if(!empty($row['id'])) $func->delete_file(UPLOAD_SEOPAGE.$row['photo']);
			}
		}
