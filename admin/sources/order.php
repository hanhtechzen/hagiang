<?php
	...
	function get_items()
	{
		...
		$id_user = (isset($_REQUEST['iduser'])) ? htmlspecialchars($_REQUEST['iduser']):0;
		$payment = (isset($_REQUEST['payment'])) ? htmlspecialchars($_REQUEST['payment']):0;

		if(!empty($tinhtrang)) $where .= " and tinhtrang=$tinhtrang";
		if(!empty($id_user)) $where .= " and id_user=$id_user";
		if(!empty($payment)) $where .= " and payment=$payment";
		if(!empty($httt)) $where .= " and httt=$httt";
		if(!empty($ngaydat))
		{
			$ngaydat = explode("to",$ngaydat);
