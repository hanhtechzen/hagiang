<?php
	...
	if(!$config['website']['debug-developer']) $func->transfer("Trang không tồn tại", "index.php", false);
	...
	function get_item()
	{
		...
		if(empty($item['id'])) $func->transfer("Dữ liệu không có thực", "index.php?com=lang&act=man&p=".$curPage, false);	
	}
	...
	function delete_item()
	{
		...
		elseif(isset($_GET['listid']))
		{
			...
			for($i=0;$i<count($listid);$i++)
			{
				...
				if(!empty($row['id'])) $d->rawQuery("delete from #_lang where id = ?",array($id));
			}
