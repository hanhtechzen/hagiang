<?php
	...
	function save_item()
	{
		...
		if(isset($config['product'][$type]['tienich']) && $config['product'][$type]['tienich']==true) 
		{
			if(!empty($_POST['tienich_group']) && $_POST['tienich_group']!='') $data['tienich'] = implode(",", $_POST['tienich_group']);
			else $data['tienich'] = "";
		}
	...
	function save_list()
	{
		...
		$file_name = $func->uploadName($_FILES['file']["name"]);
		$file_name_icon = $func->uploadName($_FILES['file-icon']["name"]);
		$id = (isset($_POST['id'])) ? (int)htmlspecialchars($_POST['id']):0;
		...
		{					
			...
			if($icon = $func->uploadImage("file-icon", $config['product'][$type]['img_type_list'], UPLOAD_PRODUCT,$file_name_icon))
			{
				$data['icon'] = $icon;
				$row = $d->rawQueryOne("select id, icon from #_product_list where id = ? and type = ?",array($id,$type));
				if(!empty($row['id'])) $func->delete_file(UPLOAD_PRODUCT.$row['icon']);
			}
	...
	function delete_list()
	{
		...
		if(isset($_GET['id']))
		{
			...
			/* Lấy dữ liệu */
			$row = $d->rawQueryOne("select id, photo,icon from #_product_list where id = ? and type = ?",array($id,$type));
