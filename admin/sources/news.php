<?php
	...
	function save_list()
	{
		...
		if(isset($id) && $id!=0)
		{
			...
			if(isset($_FILES['file-icon'])){		
				if($icon = $func->uploadImage("file-icon", $config['news'][$type]['img_type_list'], UPLOAD_NEWS,$file_name_icon))
				{
					$data['icon'] = $icon;
					$row = $d->rawQueryOne("select id, icon from #_news_list where id = ? and type = ?",array($id,$type));
					if(!empty($row['id'])) $func->delete_file(UPLOAD_NEWS.$row['icon']);
				}
			}
	...
	function delete_list()
	{
		...
		if(isset($_GET['id']))
		{
			...
			/* Lấy dữ liệu */
			$row = $d->rawQueryOne("select id, photo,icon from #_news_list where id = ? and type = ?",array($id,$type));

			if(!empty($row['id']))
			{
				$func->delete_file(UPLOAD_NEWS.$row['photo']);
				$func->delete_file(UPLOAD_NEWS.$row['icon']);
