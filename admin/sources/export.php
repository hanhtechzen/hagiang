<?php	
	...
	function exportExcel()
	{
		...
		if(isset($_POST['exportExcel']))
		{
			...
			/* Lấy dữ liệu */
			$whereCategory = "";
			$idlist = (isset($_POST['id_list'])) ? htmlspecialchars($_POST['id_list']):0;
			$idcat = (isset($_POST['id_cat'])) ? htmlspecialchars($_POST['id_cat']):0;
			$iditem = (isset($_POST['id_item'])) ? htmlspecialchars($_POST['id_item']):0;
			$idsub = (isset($_POST['id_sub'])) ? htmlspecialchars($_POST['id_sub']):0;
			if(!empty($idlist)) $whereCategory .= " and id_list=$idlist";
			if(!empty($idcat)) $whereCategory .= " and id_cat=$idcat";
			if(!empty($iditem)) $whereCategory .= " and id_item=$iditem";
			if(!empty($idsub)) $whereCategory .= " and id_sub=$idsub";
			...
			if(!empty($product)){
				...
				for($i=0;$i<count($product);$i++)
				{
					...
					foreach($array_file as $k=>$v)
					{
						if($k=='id_list')
						{
							$tenlist = $d->rawQueryOne("SELECT tenvi FROM #_product_list WHERE id = ?",array($product[$i][$k]));
							$datacell = $tenlist['tenvi'];
						}
						else if($k=='id_cat')
						{
							$tencat = $d->rawQueryOne("SELECT tenvi FROM #_product_cat WHERE id = ?",array($product[$i][$k]));
