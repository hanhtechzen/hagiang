<?php
	...
	function get_item()
	{
		...
		if(empty($id)) $func->transfer("Không nhận được dữ liệu", "index.php?com=tags&act=man&type=".$type."&p=".$curPage, false);

		$item = $d->rawQueryOne("select * from #_tags where id = ? and type = ?",array($id,$type));

		if(empty($item['id'])) $func->transfer("Dữ liệu không có thực", "index.php?com=tags&act=man&type=".$type."&p=".$curPage, false);
	}
	...
	function save_item()
	{
		...
		/* Post seo */
		if(!empty($config['tags'][$type]['seo']) && $config['tags'][$type]['seo']==true)
		{
		...
		if(isset($id) && $id!=0)
		{
			if(isset($_FILES['file'])){
				...
				{
					$data['photo'] = $photo;
					$row = $d->rawQueryOne("select id, photo from #_tags where id = ? and type = ?",array($id,$type));
					if(!empty($row['id'])) $func->delete_file(UPLOAD_TAGS.$row['photo']);
				}
			}
