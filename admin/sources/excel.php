<?php
	...
	if(!$config['order']['excel']) $func->transfer("Trang không tồn tại", "index.php", false);
	...
	if($phiship>0) $phiship = number_format($phiship, 0, ',', '.')."đ";
	else $phiship = "Không";
	...
	for($i=0;$i<count($items);$i++) 
	{
		...
		$thanhtien = 0;
		if($items[$i]['giamoi']>0) $thanhtien = number_format($items[$i]['giamoi']*$items[$i]['soluong'], 0, ',', '.')."đ";
	...
	if($config['order']['ship']==true || $config['order']['coupon']==true)
	{
		/* Tạm tính */
