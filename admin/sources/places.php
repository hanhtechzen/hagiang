<?php
	...
	if(!$config['places']['active']) $func->transfer("Trang không tồn tại", "index.php", false);
	...
	function get_city()
	{
		...
		if(empty($item['id'])) $func->transfer("Dữ liệu không có thực", "index.php?com=places&act=man_city&p=".$curPage.$strUrl, false);
	}
	...
	function delete_city()
	{
		...
		elseif(isset($_GET['listid']))
		{
			...
			for($i=0;$i<count($listid);$i++)
			{
				...
				if(!empty($row['id'])) $d->rawQuery("delete from #_city where id = ?",array($id));
			}
	...
	function get_district()
	{
		...
		if(empty($item['id'])) $func->transfer("Dữ liệu không có thực", "index.php?com=places&act=man_district&p=".$curPage.$strUrl, false);
	}
	...
	function save_district()
	{
		...
		$data = (isset($_POST['data'])) ? $_POST['data']:null;
		if(!empty($data)) { foreach($data as $column => $value) $data[$column] = htmlspecialchars($value); }
		$data['tenkhongdau'] = $func->changeTitle($data['ten']);
		$data['hienthi'] = ($data['hienthi']) ? 1 : 0;
	...
	function delete_district()
	{
		...
		elseif(isset($_GET['listid']))
		{
			...
			for($i=0;$i<count($listid);$i++)
			{
				...
				if(!empty($row['id'])) $d->rawQuery("delete from #_district where id = ?",array($id));
			}
	...
	function get_wards()
	{
		...
		if(empty($item['id'])) $func->transfer("Dữ liệu không có thực", "index.php?com=places&act=man_wards&p=".$curPage.$strUrl, false);
	}
	...
	function delete_wards()
	{
		...
		elseif(isset($_GET['listid']))
		{
			...
			for($i=0;$i<count($listid);$i++)
			{
				...
				if(!empty($row['id'])) $d->rawQuery("delete from #_wards where id = ?",array($id));
			}
	...
	function get_street()
	{
		...
		if(empty($id)) $func->transfer("Không nhận được dữ liệu", "index.php?com=places&act=man_street&p=".$curPage.$strUrl, false);
		
		$item = $d->rawQueryOne("select * from #_street where id = ?",array($id));

		if(empty($item['id'])) $func->transfer("Dữ liệu không có thực", "index.php?com=places&act=man_street&p=".$curPage.$strUrl, false);
	}
	...
	function delete_street()
	{
		...
		elseif(isset($_GET['listid']))
		{
			...
			for($i=0;$i<count($listid);$i++)
			{
				...
				if(!empty($row['id'])) $d->rawQuery("delete from #_street where id = ?",array($id));
			}
