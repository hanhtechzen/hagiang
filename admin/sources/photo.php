<?php
	...
	function save_static()
	{
		...
		$file_name = $func->uploadName($_FILES['file']["name"]);
		$row = $d->rawQueryOne("select id, options from #_photo where act = ? and type = ?",array('photo_static',$type));
		$id = $row['id'];
		...
		if(isset($id) && $id!=0)
		{
			if(isset($_FILES['file'])){
				...
				{
					$data['photo'] = $photo;
					$row = $d->rawQueryOne("select id, photo from #_photo where id = ? and act = ? and type = ?",array($id,'photo_static',$type));
					if(!empty($row['id'])) $func->delete_file(UPLOAD_PHOTO.$row['photo']);
				}
			}
			if(isset($_FILES['file2'])){
				$file_name = $func->uploadName($_FILES['file2']["name"]);
				if($photo2 = $func->uploadImage("file2", $config['photo']['photo_static'][$type]['img_type'], UPLOAD_PHOTO, $file_name))
				{
					$data['photo2'] = $photo2;
					$row = $d->rawQueryOne("select id, photo2 from #_photo where id = ? and act = ? and type = ?",array($id,'photo_static',$type));
					if(!empty($row['id'])) $func->delete_file(UPLOAD_PHOTO.$row['photo2']);
				}
			}
			if(isset($_FILES['file-video'])){
				$file_name = $func->uploadName($_FILES['file-video']["name"]);
				if($video = $func->uploadImage("file-video", $config['photo']['photo_static'][$type]['video_type'], UPLOAD_PHOTO, $file_name))
				{
					$data['video'] = $video;
					$row = $d->rawQueryOne("select id, video from #_photo where id = ? and act = ? and type = ?",array($id,'photo_static',$type));
					if(!empty($row['id'])) $func->delete_file(UPLOAD_PHOTO.$row['video']);
				}
			}
	...
	function get_photo()
	{
		...
		if(empty($item['id'])) $func->transfer("Dữ liệu không có thực", "index.php?com=photo&act=man_photo&type=".$type."&p=".$curPage, false);
	}
	...
	function save_photo()
	{
		...
		if(isset($id) && $id!=0)
		{
			if(isset($_FILES["file"])){
				...
				{
					$data['photo'] = $photo;
					$row = $d->rawQueryOne("select id, photo from #_photo where id = ? and type = ?",array($id,$type));
					if(!empty($row['id'])) $func->delete_file(UPLOAD_PHOTO.$row['photo']);
				}
			}
