<?php
	...
	if(!$config['onesignal']) $func->transfer("Trang không tồn tại", "index.php", false);
	...
	function get_man()
	{
		...
		if(empty($id)) $func->transfer("Không nhận được dữ liệu", "index.php?com=pushOnesignal&act=man&p=".$curPage, false);	

		$item = $d->rawQueryOne("select * from #_pushonesignal where id = ?",array($id));

		if(empty($item['id'])) $func->transfer("Dữ liệu không có thực", "index.php?com=pushOnesignal&act=man&p=".$curPage, false);
	}
	...
	function save_man()
	{
		...
		if(isset($id) && $id!=0)
		{
			if($photo = $func->uploadImage("file",'.jpg|.gif|.png|.jpeg|.gif|.JPG|.PNG|.JPEG|.Png|.GIF',UPLOAD_SYNC,$file_name))
			{
				...
				if(!empty($row['id'])) $func->delete_file(UPLOAD_SYNC.$row['photo']);
			}
