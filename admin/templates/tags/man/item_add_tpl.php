<?php
    ...
    if((!empty($config['tags'][$type]['images']) && $config['tags'][$type]['images']==true) || $flagGallery)
    {
...
<div class="content-inner container-fluid pb-0" id="page_layout">
    <form class="validation-form" novalidate method="post" action="<?=$linkSave?>" enctype="multipart/form-data">
        <div class="row">
            ...
            <div class="<?=$colLeft?>">
                <?php if(!empty($config['tags'][$type]['slug']) && $config['tags'][$type]['slug']==true)
                {
                ...
                <div class="card">
                    <div class="card-header">
                        <h5 class="mb-0">Quản lý <?=$config['tags'][$type]['title_main']?></h5>
                    </div>
                    <div class="card-body">
            ...
            <?php if(isset($config['tags'][$type]['images']) && $config['tags'][$type]['images']==true) { ?>
                    <?php
                        $photoDetail = (!empty($item['photo'])) ? UPLOAD_TAGS.$item['photo']:'';
                        $dimension = "Width: ".$config['tags'][$type]['width']." px - Height: ".$config['tags'][$type]['height']." px (".$config['tags'][$type]['img_type'].")";
                    ?>
                    <div class="card">
                        <div class="card-header">
                            <h5 class="mb-0">Hình đại diện <?=$config['tags'][$type]['title_main']?></h5>
                        </div>
                        <div class="card-body">
