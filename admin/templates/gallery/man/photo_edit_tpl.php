            ...
            <div class="card">
                <div class="card-header">
                    <div class="d-flex justify-content-between align-items-center flex-wrap gap-3">
                        <h5 class="mb-0 text-capitalize">Cập nhật <?=$gallery_config['title_main_photo']?></h5>
                    </div>
                </div>
                <div class="card-body">
                        ...
                        <div class="form-group">
                            <label class="change-photo" for="file-zone">
                                ...
                                <div class="rounded photoUpload-preview flex items-center justify-left " id="photoUpload-preview">
                                    <img class="rounded img-upload max-w-2xl" src="<?="../upload/".$com."/".$item['photo']?>" onerror="src='assets/images/noimage.png'" alt="Alt Photo"/>
                                </div>
                            </label>
                                            ...
                                            <div class="tab-pane fade <?=($k=='vi')?'active show':''?>" id="tabs-lang-<?=$k?>" role="tabpanel" aria-labelledby="tabs-lang-<?=$k?>-tab">
                                                ...
                                                <?php if(isset($gallery_config['mota_photo']) && $gallery_config['mota_photo']==true) { ?>
                                                    <div class="form-group">
                                                        ...
                                                        <textarea class="form-control <?=($gallery_config['mota_cke_photo'])?'form-control-ckeditor':''?>" name="data[mota<?=$k?>]" id="mota<?=$k?>" rows="5" placeholder="Mô tả (<?=$k?>)"><?=htmlspecialchars_decode(@$item['mota'.$k])?></textarea>
                                                    </div>
                                                <?php } ?>
                                                <?php if(isset($gallery_config['noidung_photo']) && $gallery_config['noidung_photo']==true) { ?>
                                                    <div class="form-group mb-0">
                                                        ...
                                                        <textarea class="form-control <?=($gallery_config['noidung_cke_photo'])?'form-control-ckeditor':''?>" name="data[noidung<?=$k?>]" id="noidung<?=$k?>" rows="5" placeholder="Nội dung (<?=$k?>)"><?=htmlspecialchars_decode(@$item['noidung'.$k])?></textarea>
                                                    </div>
                                                <?php } ?>
                                            </div>
                                            <?php }?>
