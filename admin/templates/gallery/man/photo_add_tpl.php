<?php
    ...
    $numberPhoto = $gallery_config['number_photo'];
?>
	...
	<form method="post" action="<?=$linkSave?>" enctype="multipart/form-data">
		<div class="row">
			...
			<div class="col-lg-12 mb-4 last:mb-0">
				<div class="card mb-0">
					<div class="card-header">
						<h5 class="mb-0"><?=$gallery_config['title_main_photo']." ".$stt?></h5>
					</div>
					<div class="card-body">
						<?php if($gallery_config['images_photo']) { ?>
						<div class="form-group">
							<label class="change-photo" for="file-zone<?=$i?>">
								<p> <b class="text-gray-800">Upload hình ảnh:</b> <span class="text-danger mt-2 mb-2 text-sm"><?php echo "Width: ".$gallery_config['width_photo']." px - Height: ".$gallery_config['height_photo']." px (".$gallery_config['img_type_photo'].")" ?></span></p>
						...
						<?php if(isset($gallery_config['link_photo']) && $gallery_config['link_photo']==true) { ?>
						<div class="form-group">
						...
						<?php if(isset($gallery_config['video_photo']) && $gallery_config['video_photo']==true) { ?>
							<div class="form-group">
						...
						<div class="row align-items-center">
							<div class="form-check ps-3 cursor-pointer form-switch col-2">
									...
									<input class="form-check-input l-0 ms-1 !float-none" name="dataMulti[<?=$i?>][hienthi]" checked type="checkbox" id="hienthi-checkbox" >
							</div>
						...
						<?php if((isset($gallery_config['tieude_photo']) && $gallery_config['tieude_photo']==true) || (isset($gallery_config['mota_photo']) && $gallery_config['mota_photo']==true) || (isset($gallery_config['noidung_photo']) && $gallery_config['noidung_photo']==true)) { ?>
								...
								<div class="card-body">
									...
									<?php foreach($config['website']['lang'] as $k => $v) { ?>
										<div class="tab-pane fade <?=($k=='vi')?'active show':''?>" id="tabs-lang-<?=$i?>-<?=$k?>" role="tabpanel" aria-labelledby="tabs-lang-<?=$i?>-<?=$k?>">
											<?php if((isset($gallery_config['tieude_photo']) && $gallery_config['tieude_photo']==true)) { ?>
											...
											<?php } ?>
											<?php if((isset($gallery_config['mota_photo']) && $gallery_config['mota_photo']==true)) { ?>
												<div class="form-group last:mb-0">
													<label for="mota<?=$k?><?=$i?>" class="form-label">Mô tả (<?=$k?>):</label>
													<textarea class="form-control <?=($gallery_config['mota_cke_photo'])?'form-control-ckeditor':''?>" name="dataMulti[<?=$i?>][mota<?=$k?>]" id="mota<?=$k?><?=$i?>" rows="5" placeholder="Mô tả (<?=$k?>)"><?=htmlspecialchars_decode(@$item['mota'.$k])?></textarea>
												</div>
											<?php } ?>
											<?php if((isset($gallery_config['noidung_photo']) && $gallery_config['noidung_photo']==true)) { ?>
												<div class="form-group last:mb-0">
													<label for="noidung<?=$k?><?=$i?>" class="form-label">Nội dung (<?=$k?>):</label>
													<textarea class="form-control <?=($gallery_config['noidung_cke_photo'])?'form-control-ckeditor':''?>" name="dataMulti[<?=$i?>][noidung<?=$k?>]" id="noidung<?=$k?><?=$i?>" rows="5" placeholder="Nội dung (<?=$k?>)"><?=htmlspecialchars_decode(@$item['noidung'.$k])?></textarea>
												</div>
											<?php } ?>
										</div>
									<?php }?>
