            ...
            <div class="card">
                <div class="card-header">
                    <div class="d-flex justify-content-between align-items-center flex-wrap gap-3">
                        <h5 class="mb-0 text-capitalize">Quản lý màu sắc</h5>
                            ...
                            <tbody>
                                <?php for($i=0;$i<count($items);$i++) { ?>
                                    <tr>
                                        ...
                                        <?php if(!empty($config['product'][$type]['mau_images']) && $config['product'][$type]['mau_images']==true) { ?>
                                            <td class="align-middle">
                                                <a href="<?=$linkEdit?>&id=<?=$items[$i]['id']?>" title="<?=$items[$i]['tenvi']?>"><img class="rounded img-preview" onerror="src='<?=THUMBS?>/<?=$config['product'][$type]['thumb_mau']?>/assets/images/noimage.png'" src="<?=THUMBS?>/<?=$config['product'][$type]['thumb_brand']?>/<?=UPLOAD_PRODUCT_L.$items[$i]['photo']?>" alt="<?=$items[$i]['tenvi']?>"></a>
                                            </td>
                                        <?php } ?>
                                        <td class="align-middle" >
                                        ...
                                        <td class="align-middle text-center">
                                            <div class="custom-control custom-checkbox my-checkbox form-switch">
                                                <input type="checkbox" class="form-check-input show-checkbox" id="show-checkbox-<?=$items[$i]['id']?>" data-table="product_mau" data-id="<?=$items[$i]['id']?>" data-loai="hienthi" <?=($items[$i]['hienthi'])?'checked':''?>>
