<?php
	function get_main_city()
    {
        ...
        $str = '<select id="id_city" name="id_city" onchange="onchangeList()" class="form-control select2"><option value="0">Tỉnh thành</option>';
        foreach($row as $v)
        {
    ...
    function get_main_district()
    {
        ...
        $str = '<select id="id_district" name="id_district" onchange="onchangeCat()" class="form-control select2"><option value="0">Quận huyện</option>';
        foreach($row as $v)
        {
	...
	function get_main_wards()
	{
		...
		$str = '<select id="id_wards" name="id_wards" onchange="onchangeItem()" class="form-control select2"><option value="0">Phường xã</option>';
		foreach($row as $v)
        {
            if($v["id"] == (int)$_REQUEST['id_wards']) $selected = "selected";
            else $selected = "";
            $str .= '<option value='.$v["id"].' '.$selected.'>'.$v["ten"].'</option>';
        }
        $str .= '</select>';
...
<section class="content">
    ...
    <div class="card-footer form-group-category text-sm bg-light row">
        <div class="form-group col-xl-2 col-lg-3 col-md-4 col-sm-4 mb-2"><?=get_main_city();?></div>
        <div class="form-group col-xl-2 col-lg-3 col-md-4 col-sm-4 mb-2"><?=get_main_district();?></div>
        <div class="form-group col-xl-2 col-lg-3 col-md-4 col-sm-4 mb-2"><?=get_main_wards();?></div>
    </div>
                ...
                <?php } else { ?>
                    <tbody>
                        <?php for($i=0;$i<count($items);$i++) {
                            $linkID = "";
                            if(isset($items[$i]['id_city'])) $linkID .= "&id_city=".$items[$i]['id_city'];
                            if(isset($items[$i]['id_district'])) $linkID .= "&id_district=".$items[$i]['id_district'];
                            if(isset($items[$i]['id_wards'])) $linkID .= "&id_wards=".$items[$i]['id_wards']; ?>
                            <tr>
