<?php
    function get_main_list()
    {
        ...
        $str = '<select id="id_list" name="data[id_list]" data-level="0" data-type="'.$type.'" data-table="#_news_cat" data-child="id_cat" class="select2-basic-single js-states form-control" id="select-list"><option value="0">Chọn danh mục cấp 1</option>';
        foreach($row as $v)
        {
    ...
    function get_main_cat()
    {
        ...
        $str = '<select id="id_cat" name="data[id_cat]" class="select2-basic-single js-states form-control" id="select-cat"><option value="0">Chọn danh mục</option>';
        foreach($row as $v)
        {
...
<div class="content-inner container-fluid pb-0" id="page_layout">
    <form class="validation-form" novalidate method="post" action="<?=$linkSave?>" enctype="multipart/form-data">
        <div class="row">
            ...
            <div class="col-xl-8">
                <?php if(!empty($config['news'][$type]['slug_item']) && $config['news'][$type]['slug_item']==true)
                {
                ...
                <div class="card">
                    <div class="card-header">
                        <h5 class="mb-0">Nội dung <?=$config['news'][$type]['title_main_item']?></h5>
                    </div>
                    <div class="card-body">
                                        ...
                                        <div class="tab-pane fade <?=($k=='vi')?'active show':''?>" id="tabs-lang-<?=$k?>" role="tabpanel" aria-labelledby="tabs-lang-<?=$k?>-tab">
                                            ...
                                            <?php if(isset($config['news'][$type]['noidung_item']) && $config['news'][$type]['noidung_item']==true) { ?>
                                                <div class="form-group last:mb-0">
                                                    ...
                                                    <textarea class="form-control for-seo <?=($config['news'][$type]['noidung_item_cke'])?'form-control-ckeditor':''?>" name="data[noidung<?=$k?>]" id="noidung<?=$k?>" rows="5" placeholder="Nội dung (<?=$k?>)"><?=htmlspecialchars_decode(@$item['noidung'.$k])?></textarea>
                                                </div>
                                            <?php } ?>
                                        </div>
                                        <?php }?>
            ...
            <div class="col-xl-4">
                ...
                <?php if(isset($config['news'][$type]['images_item']) && $config['news'][$type]['images_item']==true) { ?>
                    ...
                    <div class="card">
                        <div class="card-header">
                            <h5 class="mb-0">Hình đại diện <?=$config['news'][$type]['title_main_item']?></h5>
                        </div>
                        <div class="card-body">
        ...
        <div class="card">
            <div class="card-header">
                <h5 class="mb-0">Hình ảnh đính kèm <span class="text-danger mt-2 mb-2 text-sm">(<?=(!empty($config['news'][$type]['gallery_item'][$key]['img_type_photo'])) ? $config['news'][$type]['gallery_item'][$key]['img_type_photo']:''?>)</span></h5>
            </div>
            <div class="card-body">
                <div class="form-group last:mb-0">
