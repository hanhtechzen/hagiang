            ...
            <div class="card">
                <div class="card-header">
                    <div class="d-flex justify-content-between align-items-center flex-wrap gap-3">
                        <h5 class="mb-0 text-capitalize"><?=$config['news'][$type]['title_main_list']?></h5>
                        ...
                        <table class="table mb-0 w-100" id="datatable">
                            <thead>
                                <tr class="bg-white">
                                    ...
                                    <?php if(!empty($config['news'][$type]['show_images_list']) && $config['news'][$type]['show_images_list']==true) { ?>
                                        <th class="align-middle">Hình</th>
                                    <?php } ?>
                                    <th scope="col" style="width:70%;min-width: 200px">Tiêu đề</th>
                                    <?php if(!empty($config['news'][$type]['gallery_list']) && count($config['news'][$type]['gallery_list']) > 0) { ?>
                                        <th scope="col">Gallery</th>
                                    <?php } ?>
                                    <?php if(!empty($config['news'][$type]['check_list'])){ foreach($config['news'][$type]['check_list'] as $key => $value) { ?>
                            ...
                            <tbody>
                                <?php for($i=0;$i<count($items);$i++) { ?>
                                    <tr>
                                        ...
                                        <?php if(!empty($config['news'][$type]['check_list'])){ foreach($config['news'][$type]['check_list'] as $key => $value) { ?>
                                            <td class="align-middle text-center">
                                                <div class="custom-control custom-checkbox my-checkbox form-switch">
                                                    <input type="checkbox" class="form-check-input show-checkbox" id="show-checkbox-<?=$key?>-<?=$items[$i]['id']?>" data-table="news_list" data-id="<?=$items[$i]['id']?>" data-loai="<?=$key?>" <?=($items[$i][$key]==true)?'checked':''?>>
                                        ...
                                        <td class="align-middle text-center">
                                            <div class="custom-control custom-checkbox my-checkbox form-switch">
                                                <input type="checkbox" class="form-check-input show-checkbox" id="show-checkbox-<?=$items[$i]['id']?>" data-table="news_list" data-id="<?=$items[$i]['id']?>" data-loai="hienthi" <?=($items[$i]['hienthi'])?'checked':''?>>
