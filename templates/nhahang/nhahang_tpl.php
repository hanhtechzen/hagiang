...
<section class="wrap-order-tour bg-[#2C2C2C]" >
	<div class="container flex items-center justify-between">
			...
			<div class="show-ul-list" @click="show_list = !show_list" @click.outside="show_list = false">
				...
				<div class="bolck-show-list absolute top-full mt-[10px] left-0 z-[888] w-full bg-white " x-show="show_list">
					<ul class="data-scrollbar max-h-[400px]">
						...
						<li><a :class="(nhahang_select == <?=$v['id']?>)?'!bg-[#E2F7DD] text-[#3A9923]':''" @click="nhahang_select = <?=$v['id']?>,title_select=`<?=$v['ten']?>`" class="block text-[#1A1111] text-[16px] px-[10px] py-[8px] transition-all duration-500 hover:bg-[#E2F7DD] hover:text-[#3A9923]" href="javascript:void(0)" data-test="<?=$v['ten']?>"><?=$v['ten']?></a></li>
						<?php }?>
					</ul>
				</div>
			</div>
		...
		<div class="bloxk-search w-[180px]">
			<a href="javascript:void(0)" data-toggle="modal" data-target="#exampleModal" class="bg-[#3A9923] uppercase font-bold text-white text-[16px] py-[44px] px-[20px] !text-center block">ĐẶT BÀN NGAY</a>
		</div>
	</div>
</section>
<section class="detail-tour bg-[#F4F4F4] py-[50px]">
	<div class="container">
		<div class="title-chill-tour text-center mb-[30px]">
			<p class="uppercase text-[40px] capitalize font-Viaoda">Nhà hàng nổi bật</p>
		</div>
	...
	<div class="wrap-daxem mt-[50px]">
		<div class="container">
			<div class="title-chill-tour text-center mb-[30px]">
				<p class="uppercase text-[40px] capitalize font-Viaoda">Nhà hàng đã xem</p>
			</div>
			...
			<div class="modal-body relative">
				...
				<div class="wrap-show-form">
					<form id="form-recaptchaResponseDatban" method="post" action="" enctype="multipart/form-data">
						...
						<input type="hidden" name="data[id_nhahang]" x-model="nhahang_select">
						<input type="hidden" name="url" value="<?=$func->getCurrentPageURL()?>">
						<input type="hidden" name="data[name_nhahang]" x-model="title_select">
